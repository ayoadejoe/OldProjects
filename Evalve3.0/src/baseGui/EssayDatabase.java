...
public class EssayDatabase {
	...
	public void loadEssay(String ClassS, String Subject, String Examday, String Examtime, String Duration, String Terms, String Examiner,
			String Exam, int newNoQ, int pointing, int Alt, int pt, String ExamType,String EvalType) throws FileNotFoundException{
		String ESubject = "Essay"+Subject;
		Scanner baseCheck;
		String datB = ESubject;
			baseCheck = new Scanner(new File("DatabaseList.txt"));
			while(baseCheck.hasNextLine()){
				//(ESubject+" == "+datB);
			     if(datB.equals(baseCheck.nextLine().trim())){
			        //("Database Exists");
			        loadConfirmEssay(ClassS, ESubject, Examday, Examtime, Duration, Terms, Examiner,
							Exam, newNoQ, pointing, Alt, pt, ExamType, EvalType );
			        return;
			      }
			}
			Scanner baseCheck2;
			String datB2 = ESubject;
			...
			while(baseCheck2.hasNextLine()){
			     if(datB2.equals(baseCheck2.nextLine().trim())){
			        ...
			        return;
			    	  ...
			    	  createDataBEssay(ClassS, ESubject, Examday, Examtime, Duration, Terms, Examiner,
								Exam, newNoQ, pointing, Alt, pt, ExamType, EvalType );
			    	  return;
			      }
			}
	...
	public void loadConfirmEssay(String ClassS, String ESubject, String Examday, String Examtime, String Duration, String Terms, String Examiner,
		...
		while (codeRetriever.hasNext()){
			...
			String DATABASE_URL = "jdbc:mysql://"+IP+":"+port+"/"+ESubject; 
	...
	public void createDataBEssay(String ClassS, String ESubject, String Examday, String Examtime, String Duration, String Terms, String Examiner,
			String Exam, int newNoQ, int pointing, int Alt, int pt, String ExamType,String EvalType){
	//("Check: Database does not Exists");
	  try {
		Class.forName(JDBC_DRIVER);
	//("Connected for new database.");
	Connection connection;
	Scanner codeRetriever = new Scanner(new File("data.ini"));
	while (codeRetriever.hasNext()){
		String USERNAME = codeRetriever.next();
		...
		connection = DriverManager.getConnection(DATABASE_URL, USERNAME, PASSWORD);
		Statement stmt = connection.createStatement( );
		String createsql = "CREATE DATABASE "+ESubject+";";
		stmt.executeUpdate(createsql);
		String DATABASE_URL2 = "jdbc:mysql://"+IP+":"+port+"/"+ESubject; 
		connection = DriverManager.getConnection(DATABASE_URL2, USERNAME, PASSWORD);
		//("Connection to new database "+ESubject);

		for (int d = 1; d<=3; d++){
			String clas = "jss"+d;
			
		String createTables = "CREATE TABLE `"+ESubject+"`.`"+clas+"` (`Questions` VARCHAR(200) NULL," +
				"`EvalType` VARCHAR(50) NULL,`Diagram` VARCHAR(45) NULL," +
				...
				"`Term` VARCHAR(11) NULL, `ExamType` VARCHAR(20) NULL,`No` INT(100) NOT NULL AUTO_INCREMENT,PRIMARY KEY (`No`));;";
      ...
      //(ESubject+" Tables created successfully");
	    	  stmt.close();                        
	          connection.close(); 
	}
	} catch (ClassNotFoundException e) {
