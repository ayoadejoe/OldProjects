...
public class SelectionExam extends JDialog{
	private Color bakG2;
	...
	public SelectionExam(JFrame parent,final int IDNo, final String Work, final String Username, final String SubClassCl,
			final String term) {
		super(parent, "EXAM SETTINGS", false);
		try {
		...
		major.add(subject+"   ,"+clazz+"      ,"+cl);
		q = q+1;u++;
		}
		}catch(ArrayIndexOutOfBoundsException f){}
		Object[] Typelist = major.toArray();
		
		Font font = new Font("SERIF", Font.BOLD, 17);
		
		schd = new JLabel("Paper");
		schd.setFont(font);
		align.insets = new Insets(0, 0, 70, 0);		// format: push(DOWN, right, UP, left)
		...
		evType = new JList(Typelist);
		evType.setVisibleRowCount(2);
		evType.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		...
		align.weighty = 0;
		align.fill = GridBagConstraints.NONE;
		add(new JScrollPane(evType), align);
		
		Font but = new Font("SERIF", Font.BOLD, 16);
		entr.setFont(but);
		entr.setForeground(Color.RED);
		header = new JLabel("SCHEDULE FOR EXAM");
		...
		entr.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ev1) {
				...
				String EvalType = (String) evType.getSelectedValue();
				String[] parts = EvalType.split(",");
				String subject = parts[0].trim().toUpperCase();
				String clazz = parts[1].trim(); 
				String cl = parts[2].trim();
				
				String ClassS = clazz;
				String Subject = subject;
				String creditLoad = cl;
				...
				int pt = 1;
				String Topic = subject+" EXAMINATION";
				int Week = 10;
				
				if(Work.equals("Exam")){
				if(ExamType.equals("M.C.Q")){
					new mcqEngine(IDNo, NoQ, pointing, weighting, 0, 
							Work, Username, ClassS, Subject, Examday, Examtime, Duration, Terms, Examiner, Topic, null, Week);
