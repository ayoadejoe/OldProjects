...
public class BulkGraphDesign extends JPanel{
            ...
            if(graphData.size()>0){								
            	...
            	for(int u =0; u<xtickNo; u++){		//if the no of readings is much decrease ticks
            		...
            		if(graphData.get(u) instanceof Integer){		//start(x1,y1 --- end x2,y2)
            			...
            			}catch(IndexOutOfBoundsException a){System.out.println(a.getMessage());}
            			z++;
            			g2.setColor(Color.RED);
            			//System.out.println(xlineSpace+","+ (getHeight()-ydataVariables.get(z))+","+ (xlineSpace+(xtickSpace*2))+","+ (getHeight()-ydataVariables.get(z+1)));
            		}
            		
            		if(graphData.get(u).toString().contains("WAT")){
            			g2.setColor(Color.WHITE);
            			g2.setFont(new Font("cambria_math", Font.BOLD, 14));
            			today = (Date) graphData.get(u);
            			g2.drawString("Data for "+today.toLocaleString(), (int)getWidth()/2, 80);
            			g2.setColor(Color.RED);
            		}
            		
            		
            		if(u == mlineSpace){
            		mlineSpace=mlineSpace+mSpace;
            		if(graphData.get(u) instanceof String){
            			g2.drawLine(mlineSpace, getHeight()-45, mlineSpace, getHeight()-55);
            			g2.drawString(graphData.get(u).toString(), mlineSpace, getHeight()-30);
            		}else{
            			g2.drawLine(mlineSpace, getHeight()-45, mlineSpace, getHeight()-55);
            			if(graphData.get(u+1) instanceof String){
            			g2.drawString(graphData.get(u+1).toString(), mlineSpace, getHeight()-30);
            			}
            		}
            		}
            	}
	...
	public BulkGraphDesign(List<Object> data2) {
		...
		//.out.println("y data Size:"+ydataVariables.size());
		this.xtickNo = ydataVariables.size();
	}
	...
	private static BulkGraphDesign graph;
	...
	public static void main(String[] args) {
		...
		graph = new BulkGraphDesign(defaulT);
		...
		refresh.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				frame.remove(graph);
				graph = new BulkGraphDesign(data);
